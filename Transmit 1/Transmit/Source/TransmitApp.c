...
//---pan
  ...
  static uint8 alarm = 0;
  static uint8 * UART_msg = NULL;  
...
void TransmitApp_Init( byte task_id )
{
  ...
  ZDO_RegisterForZDOMsg( TransmitApp_TaskID, Match_Desc_rsp );
    ...
    uart_sent( "Home Automation --Start!!\n",0);
}
...
//            }
  ...
  // DEBUG INFO
  if ( events & TRANSMITAPP_UART_EVT )
  {
    //TransmitApp_UART_Evt();
    
    if ( UART_msg )
    {
      HalUARTWrite(ZTOOL_PORT, UART_msg+1, UART_msg[0]);
      osal_mem_free((uint8 *)UART_msg);
    }
    
    // Return unprocessed events
    return (events ^ TRANSMITAPP_UART_EVT);
  }
...
void TransmitApp_ProcessZDOMsgs( zdoIncomingMsg_t *inMsg )
{
  switch ( inMsg->clusterID )
  {
    case End_Device_Bind_rsp:
      ...
      { uart_sent("Bind Success!\n",0);
        // Light LED
        HalLedSet( HAL_LED_2, HAL_LED_MODE_ON );
      }
#if defined(BLINK_LEDS)
      else
      { uart_sent("Bind failure!\n",0);
    ...
    case Match_Desc_rsp:
      {
        ...
        if ( pRsp )
        {
          ...
          { 
            ...
            uart_sent("Match_Desc_rsp Sucess!\n",0);
            // Light LED
            HalLedSet( HAL_LED_2, HAL_LED_MODE_ON );
          }
          osal_mem_free( pRsp );
...
void TransmitApp_HandleKeys( byte shift, byte keys )
{
  ...
  {
    if ( keys & HAL_KEY_KEY_1 )
    {   //uart_sent( "Initiate a Match Description Request (Service Discovery) \n",0);
      uart_sent("<",0);
//      rxTotal = txTotal = 0;
//      rxAccum = txAccum = 0;
//      TransmitApp_DisplayResults();
...
//                        FALSE );      
    ...
    if ( keys & HAL_KEY_KEY_3 )     //开关灯
    {
      light = !light;

      if(light)
      { 
       uart_sent("sent light on cmd---\n",0);
      }
      else
      {
        uart_sent("sent light off cmd---\n",0);
      }
    ...
    if ( keys & HAL_KEY_KEY_4 )     //报警
    { alarm = !alarm;

      if(alarm)
      {  
       uart_sent("sent alarm on cmd---\n",0);
      }
      else
      {
        uart_sent("sent alrm off cmd---\n",0);
      }       
...
//----------------------------      
      //灯光
      if (pkt->cmd.Data[4] == 100)
      {P1DIR |= 0x01;
       P1    |= 0x01;
      }
      else if (pkt->cmd.Data[4] == 0) //关灯
      {
       P1DIR |= 0x01;
       P1    &= ~0x01;
      }
      
      if (pkt->cmd.Data[5] == 1)//报警
      {P1DIR |= 0x02;
       P1    |= 0x02;
      }
      else if (pkt->cmd.Data[5] == 0) 
      {
       //P1DIR &= ~0x02;
       P1    &= ~0x02;
      }      
      ...
      uint8 *buf;
     // uint8 tmp;
      buf = (uint8 *)osal_mem_alloc( 27 );
      if(buf)
      {
        for( i =0; i <6 ;i++)
        {
          buf[i*5+2] = '0'+(pkt->cmd.Data[i])/100;
          buf[i*5+3] = '0'+(pkt->cmd.Data[i]/10)%10;
          buf[i*5+4] = '0'+(pkt->cmd.Data[i])%10;
        }
        ...
        //HalLcdWriteString( (uint8*)buf, HAL_LCD_LINE_2 );
        //debug_str( (uint8*)buf );
        uart_sent( (uint8*)buf ,27);
        /* Free mem */
        osal_mem_free( buf );
      }
//----------------------------
...
//
    ...
    {TransmitApp_Msg[0] = 0;
      ...
      //温度
      TransmitApp_Msg[1] = 20;

     //湿度
      TransmitApp_Msg[2] = 35;
      
      //可燃
      TransmitApp_Msg[3] = 10;
      
      //灯光
      P0DIR &= ~ 0x40;
      if(light)
      {TransmitApp_Msg[4] = 100;   
      }
      else
      {
        TransmitApp_Msg[4] = 0;
   ...
   //uart_sent("sent AF_DataRequest --- ",0);
...
//
  ...
  { uart_sent("sent Error, so wait (10 mSec) and try again.\n",0);
    // Error, so wait (10 mSec) and try again.
    osal_start_timerEx( TransmitApp_TaskID, TRANSMITAPP_SEND_ERR_EVT, 500 );
  }
}
...
void uart_sent( uint8 * msg ,uint8 len)
{   
    debug_str(msg);
}
/*********************************************************************
